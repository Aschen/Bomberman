...
bool Socket::start(void)
{
    ...
    {
        _error = ("Can't start Socket of type Socket::SERVER_HANDLE");
        return false;
    }
    ...
    if (_socket < 0)
    {
        _error = (std::string("Can't create socket : ") + std::string(strerror(errno)));
        return false;
    }
    ...
    if (_type == Socket::SERVER)
    {
        ...
        {
            _error = (std::string("Can't bind socket : ") + std::string(strerror(errno)));
            return false;
        }
        ...
        {
            _error = (std::string("Can't listen on socket : ") + std::string(strerror(errno)));
            return false;
        }
        _active = true;
    }
    else if (_type == Socket::CLIENT)
    {
        ...
        {
            _error = (std::string("Can't connect to host : ") + std::string(strerror(errno)));
            return false;
        }
        _active = true;
    }
...
bool Socket::stop(void)
{
    if (_active == false)
    {
        _error = (std::string("Socket is not running."));
        return false;
    }
...
bool Socket::sendMsg(int fd)
{
    ...
    if (frame.size() == 0)
    {
        _error = (std::string("No message to send in socket."));
        return false;
    }

    if (_debug)
        std::cout << "Send : " << frame.c_str() << " to socket " << fd << std::endl;
    /* On envoi le maximum du message dans la socket */
    if ((ret = write(fd, frame.c_str(), frame.size())) < 0)
    {
        _error = (std::string("Can't write on socket : ") + std::string(strerror(errno)));
        return false;
    }
...
int Socket::receiveMsg(int fd)
{
    ...
    {
        _error = (std::string("Can't read on socket : ") + std::string(strerror(errno)));
        return -1;
    }
    ...
    if (_debug)
        std::cout << "Receive " << buf << " from socket " << fd << std::endl;
...
bool Socket::addMessage(Message msg)
{
    ...
    {
        _error = (std::string("You can't push an empty message to the queue."));
        return false;
    }
    if (_debug)
        std::cout << "Add message " << msg << " to queue" << std::endl;
...
bool Socket::getMessage(Message &msg)
{
    ...
    {
        _error = (std::string("No pending message in the queue."));
        return false;
    }
    msg = _readQueue.front();
    if (_debug)
        std::cout << "Get message " << msg << " from queue " << std::endl;
...
bool Socket::acceptConnection(Socket &socket)
{
    ...
    if (_type != Socket::SERVER)
    {
        _error = (std::string("Only server type Socket can accept connection."));
        return false;
    }
    client_socket = accept(_socket, (struct sockaddr*)&handle, &handle_len);
    if (client_socket < 0)
    {
        _error = (std::string("Can't accept new connection : ") + std::string(strerror(errno)));
        return false;
    }
...
bool Socket::setOption(int optName, int level)
{
    ...
    {
        _error = (std::string("Can't set socket option : ") + std::string(strerror(errno)));
        return false;
    }
    return true;
}
...
bool Socket::setProtocol(std::string protoName)
{
    ...
    if (!protocol)
    {
        _error = (std::string("Protocol : " + protoName + " incorrect."));
        return false;
    }
    _protocol = protocol;
    return true;
...
bool Socket::setInterface(std::string interface)
{
    ...
    {
        _error = (std::string("Invalid interface : " + _interface));
        return false;
    }
...
bool Socket::setRemoteIp(std::string remoteIp)
{
    ...
    {
        _error = (std::string("You can't change your remote ip."));
        return false;
    }
    _remoteIp = remoteIp;
    return true;
...
const std::string &Socket::getRemoteIp(void)
{
    if (_type == Socket::SERVER)
    {
        _error = (std::string("No remote IP available."));
    }
    return _remoteIp;
}
...
bool Socket::set(int socket)
{
    if (_type != SERVER_HANDLE)
    {
        _error = (std::string("Use start() to open socket."));
        return false;
    }
    if (_active == false)
    {
